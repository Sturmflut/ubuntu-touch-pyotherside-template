...
import ubuntu-touch-pyotherside-template
...
class BaseTestCase(AutopilotTestCase):
    ...
    click_package = '{0}.{1}'.format('ubuntu-touch-pyotherside-template', 'sturmflut')

    def setUp(self):
        super(BaseTestCase, self).setUp()
        self.launcher, self.test_type = self.get_launcher_and_type()
        self.app = ubuntu-touch-pyotherside-template.TouchApp(self.launcher(), self.test_type)
