...
# 1) run make once to create and update the po/ubuntu-touch-pyotherside-template.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/ubuntu-touch-pyotherside-template.pot po/en.po
# 3) edit the po file
...
APP_ID=ubuntu-touch-pyotherside-template.sturmflut
...
all: po/ubuntu-touch-pyotherside-template.pot
...
po/ubuntu-touch-pyotherside-template.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/ubuntu-touch-pyotherside-template.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
